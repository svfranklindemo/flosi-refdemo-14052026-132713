...
async function fetchNews(edgeDataPath, maxItems) {
  try {
    ...
    return raw.slice(0, maxItems).map((item) => ({
      ...
      image: item.media?.value || item.media || '',
...
function buildBigCard(news, detailBasePath) {
  const href = resolveLink(news.slug, detailBasePath);
  const card = document.createElement('a');
  card.className = 'nf-card nf-card-big';
  card.href = href;

  const imgWrap = document.createElement('div');
  imgWrap.className = 'nf-card-image';
  if (news.image) {
  ...
  meta.textContent = timeAgo(news.createdAt);
