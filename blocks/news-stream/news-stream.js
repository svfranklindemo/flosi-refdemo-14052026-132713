const DEBUG_VERSION = 'news-stream-clean-v1';
...
function renderNews(items, config, container) {
  container.innerHTML = '';
  if (!items.length) {
    container.append(createElement('div', 'news-stream-empty', config.emptyStateText));
    return;
  }
...
export default async function decorate(block) {
  ...
  block.innerHTML = '';
  block.classList.add('news-stream');

  const header = createElement('div', 'news-stream-header', `
    <h2 class="news-stream-title">${title}</h2>
    <p class="news-stream-subtitle">${subtitle}${isAuthorRuntime() ? ` (${DEBUG_VERSION})` : ''}</p>
  `);
  const list = createElement('div', 'news-stream-results');
  list.innerHTML = '<p class="news-stream-loading">Carregando notícias...</p>';
  ...
  if (!isAuthorRuntime()) {
    list.innerHTML = '<p class="news-stream-error">Bloco de demo disponível somente no Author.</p>';
    return;
  }
  ...
  try {
    ...
    list.innerHTML = `<p class="news-stream-error">Erro ao carregar notícias: ${e?.message || 'unknown'}</p>`;
  }
}
